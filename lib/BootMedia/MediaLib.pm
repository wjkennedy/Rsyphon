...
sub populate_disk_image {
    ...
    my $mount_point = File::Temp::tempdir("/tmp/rsyphon.tmp.XXXXX", 
					  CLEANUP => 1);
    my $status = 0;
...
sub run_mkisofs {
    ...
    $mkisofs_cmd .= " -A \"Rsyphon $arch autoinstallcd v$version\"";
    $mkisofs_cmd .= " -V \"Rsyphon $arch Boot CD\"";
    $mkisofs_cmd .= ' -p "Created by rsmkbootmedia -- part of Rsyphon.';
    $mkisofs_cmd .= ' http://rsyphon.org/"';
