...
package Rsyphon::UseYourOwnKernel;
...
sub create_uyok_initrd() {
        ...
        $SIG{__DI_} = sub {
            my $msg = shift;
            if ($staging_dir) {
                if ($staging_dir =~ m/^\/tmp\/\.rsyphon\.[0-9]+$/) {
        ...
        # Copy template over
        #
        unless (-d "/usr/share/rsyphon/boot/$arch/standard/initrd_template/") {
            rmdir($staging_dir);
            die("FATAL: couldn't find a valid initrd template.\n" . 
                "\tTry to install rsyphon-${arch}initrd_template package!\n");
        }
        $cmd = qq(rsync -a /usr/share/rsyphon/boot/$arch/standard/initrd_template/ $staging_dir/);
        ...
        # module that someone needs. -B-
        #
        my $modules_to_exclude = '';
        $file = "/etc/rsyphon/UYOK.modules_to_exclude";
        if(-e $file) {
            #
            # Get list of exclusions from "/etc/rsyphon/UYOK.modules_to_exclude"
            # (that file should live in the "rsyphon-common" package)
            #
        ...
        if ($custom_kernel) {
            ...
            unless (-d "$image_dir/$image") {
                print STDR "error: $image is not a valid image! use rslsimage to see the list of available images.\n";
                print STDR "Remember: the option --image can be used only on the image server.\n";
                exit(1);
            }
        ...
        if ($module_dir) {
            ...
            unless ($my_modules) {
                # FIXM option L is required in order to copy all modules, even ones that are outside the
        ...
        if ($ssh_key) {
            ...
            print ">>> uding SSH private key: $ssh_key\n" if ($verbose);
        ...
        if ($authorized_keys) {
            ...
            print ">>> uding SSH authorized keys: $authorized_keys\n" if ($verbose);
        ...
        #
        if ($local_cfg) {
            print ">>> uding local.cfg into the initrd.img: $local_cfg\n" if ($verbose);
...
sub is_kernel {
        ...
        #  goal here is to make reasonable effort to _eliminate_
        ...
        #
        # refore, we should add elimination tests to this function whenever
        ...
        #
        # Get output from "file" for elimination  identification tests
        my $cmd = "file -bz $file";
        open(INPUT,"$cmd|") or die("Couldn't run $cmd to get INPUT");
                ...
                if( $input =~ m/nux Compressed ROM File System data,/ ) { return undef; }
        close(INPUT);

        #
        # If we've made it down to here, then we'll assume it's a kernel. -B-
        return 1;
}
...
sub _get_kernel_release($) {
        ...
        binmode(IN);
                # 
                # ample entries like what we're trying to match against in kernels:
                #       2.6.10bef1 (finley@mantis) #1 Tue Mar 1 00:37:55 CST 2005
                #       2.4.21.SuS273.bef1 (root@tg-c025) (gcc version 3.2.2) #1 SMP Mon Jan 24 11:55:28 CST 2005
                #       2.4.24 (root@mantis) #2 Fri Jan 16 19:51:43 CST 2004^
                #       2.4.19-mantis-.11.20 (root@mantis) #6 Tue Nov 19 15:15:43 CST 
                #       2.6.7-1-686 (dilinger@toaster.hq.voxel.net) #1 Thu Jul 8 05:36:53 T 2004
                #       2.6.22.5-31-default (geeko@buildhost) #1 SMP /09/21 22:29:00 UTC
                #
                my $regex =
...
sub is_initrd
{
        ...
        # boot the image - used  kexec stuff to generate a valid
        ...
        open(INPUT,"$cmd|") or die("Couldn't run $cmd to get INPUT");
                my ($input) = (<INPUT>);
                # eliminate vmlinux files
                if( $input =~ m/F (32|64)-bit [ML]SB/ ) { return undef; }
                # eliminate kernels
        ...
        if ($kernel_release) {
            ...
            #  kernel version string couldn't be found in the initrd, but if
...
sub _choose_initrd_file
{
        ...
        # kernel release - this function is used  kexec stuff to
...
sub choose_file_system_for_new_initrd() {
        ...
        #  auto-detection of a valid filesystem works only on the golden
        ...
        open(FILYSTS,"<$file") or die("Couldn't open $file for reading.");
        while (<FILYSTS>) {
                chomp;
        ...
        return $fs;
OUT:
        print STDR "WARNING: couldn't find a valid filesystem for initrd.img!\n";
        print STDR "WARNING: trying with cpio initramfs...\n";
        return 'cpio';
}
...
sub get_load_ordered_list_of_running_modules() {
        ...
        my $mandatory_modules_file = '/etc/rsyphon/UYOK.modules_to_include';
        my @modules = ();
        my @mandatory_modules = ();

        unless (-e $file) {
            print STDR qq(WARNING: running kernel doesn't support loadable modules!\n);
            return @modules; 
        }
        if (-e $mandatory_modules_file) {
            #
            # Get list of inclusions from "/etc/rsyphon/UYOK.modules_to_include"
            # (that file should live in the "rsyphon-common" package)
            #
            open(MODUL, "<$mandatory_modules_file") or
        ...
        if ($uname_r =~ /(^2\.6)|(^3\.[0-9]+)/) {
            ...
            die "ROR: unsupported kernel $uname_r!\n";
        }

        # get the list of the loaded module filenames.
        open(MODUL,"<$file") or die("Couldn't open $file for reading.");
        while(<MODUL>) {
                my ($module) = split;
                chomp(my $module_file = `$modinfo_filename $module 2>/dev/null`);
                if ($?) {
                        print STDR qq(WARNING: Couldn't find module "$module" (skipping it)!\n);
                        next;
        ...
        foreach my $module (@mandatory_modules) {
                chomp(my $module_file = `$modinfo_filename $module 2>/dev/null`);
                if ($?) {
                        print STDR qq(WARNING: Couldn't find module "$module", assuming it's built into the kernel.\n);
                        next;
                }
                push (@modules, $module_file);
                ...
                if ($uname_r =~ /(^2\.6)|(^3\.[0-9])/) {
                    ...
                    open(MODUL_D, "</lib/modules/$uname_r/modules.dep") or
                        die "ROR: cannot open modules.dep!\n";
                    while ($_ = <MODUL_D>) {
                        if ($_ =~ m/$module_file:(.*)$/) {
                            $_ = $1;
                            do {
                ...
                foreach (@deps) {
                    ...
                    if ($?) {
                        print STDR qq(WARNING: Couldn't find module "$_", assuming it's built into the kernel.\n);
                        next;
                    }
                    push (@modules, $module_file);
                }
...
sub _create_new_initrd($$) {
        ...
        print ">> aluating initrd size to be added in the kernel boot options\n" .
              ">> (e.g. /etc/rsyphon/pxelinux.cfg/syslinux.cfg):\n";
...
sub _create_initrd_reiserfs($$) {
        ...
        my $journal_size = $journal_blocks * 4; # journal size in blocks * block size in kilotes
...
sub _create_initrd_jfs($$) {
        ...
        chomp(my $size = `du -ks $staging_dir`);
        $size =~ s/\s+.*$//;
#        my $breathing_room = 3072;      # We may need to tweak this -- not for size(<), but for size(>). -B-
        my $breathing_room = 4072;      # We may need to tweak this -- not for size(<), but for size(>). -B-
        $size = $size + $breathing_room;
        #
        # jfs_mkfs farts on you with an "Partition must be at least 16 megates."
