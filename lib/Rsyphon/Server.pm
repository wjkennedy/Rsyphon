...
#use lib "USR_PRIX/lib/rsyphon/perl";
use Carp;
use strict;
use File::Copy;
use File::Path;
...
use vars qw($VSION @mount_points %device__mount_point %filesystem_type__mount_point $disk_no %dev2disk $bootdev $rootdev);

$VSION="SYSTIMAG_VSION_STRING";
...
sub copy_boot_files_from_image_to_shared_dir {
    ...
    my $kernel = $image_dir . "/etc/rsyphon/boot/kernel";
    my $initrd = $image_dir . "/etc/rsyphon/boot/initrd.img";
    my $file   = $image_dir . "/etc/rsyphon/boot/ARCH";
...
sub record_image_retrieved_from {
    ...
    my $file = $image_dir . "/etc/rsyphon/IMAGRRI_FROM";

    local *FIL
    open(FIL">$file") or die("Couldn't open $file for writing!");
        print FIL"$golden_client\n";
    close(FIL;
...
sub record_image_retrieval_time {
    ...
    my $file = $image_dir . "/etc/rsyphon/IMAGRRIAL_TIM;

    local *FIL
    open(FIL">$file") or die("Couldn't open $file for writing!");
        printf(FIL"%04d.%02d.%02d %02d:%02d\n", $year,$mon,$mday,$hour,$min);
    close(FIL;
...
sub gen_rsyncd_conf {
    ...
    open(RSYNC_CONF, ">$rsyncconf") or return undef;
      foreach my $stub_file (@stubfiles) {
        ...
        if ( -f $file ) {
          open(STUBFIL "<$file") or return undef;
          while (<STUBFIL) {
            print RSYNC_CONF;
          }
          close STUBFIL
...
sub validate_post_install_option {
  ...
  unless(($post_install eq "beep") or ($post_install eq "reboot") or ($post_install eq "shutdown") or ($post_install eq "kexec")) { 
    die qq(\nROR: -post-install must be beep, reboot, shutdown, or kexec.\n\n       Try "-help" for more options.\n);
  }
  return 0;
}
...
sub validate_ip_assignment_option {
  ...
  ) { die qq(\nROR: -ip-assignment must be dhcp, static, or replicant.\n\n       Try "-help" for more options.\n); }
  return 0;
}
...
sub get_image_path {
    ...
    open (FIL "<$stub_dir/40$imagename") or return undef;
    while (<FIL) {
        if (/^\s*path\s*=\s*(\S+)\s$/) {
            close FIL
            return $1;
...
# Usage:
# my $path = Rsyphon::Server->get_image_path( $rsync_stub_dir, $image );   #XXX
sub get_full_path_to_image_from_rsyncd_conf {
    ...
    print 'Rsyphon::Server->get_image_path( $rsync_stub_dir, $image );' . "\n";
    die;
}
...
sub get_all_devices($) {
    ...
    my $part_config = XMn($file, keyattr => { fsinfo => "+line" }, forcearray => 1 );
    ...
    my $fs_config = XMn($file, keyattr => { disk => "+dev", part => "+num" }, forcearray => 1 );  
...
#  standard /dev file name includes /dev/ida/c0d0 for the disk,
# while the devfs name it exports is /dev/ida/c0d0/disc.   overlapping
...
sub _read_partition_info_and_prepare_parted_commands {
    ...
    my $xml_config = XMn($file, keyattr => { disk => "+dev", part => "+num" }, forcearray => 1 );  
    ...
    #
    # Ok.  Now that we've read all of the partition scheme info into hashes, let's do stuff with it. -B-
    #
    foreach my $dev (sort (keys ( %{$xml_config->{disk}} ))) {
        ...
        print $out "### BIN partition $devfs_dev ###\n";
        ...
        print $out q(DISK_SIZ`LC_ALL=C parted -s ) . $devfs_dev . q( unit MB print | egrep ") . $devfs_dev . q(" | awk '{print $NF}' | sed 's/MB//' `) . qq(\n);
        print $out q([ -z $DISK_SIZ] && shellout) . qq(\n);

        print $out q(if [ "$ARCH" = "alpha" ]; then) . qq(\n);	
        print $out q(    D_OF_LAST_PRIMARY=1) . qq(\n);
        print $out q(else) . qq(\n);
        print $out q(    D_OF_LAST_PRIMARY=1 # 1: room for grub2) . qq(\n);
        print $out q(fi) . qq(\n\n);

        ### BIN Populate the simple hashes. -B- ###
        my (
            %end_of_disk,
            %flags,
            %id, 
        ...
        ########################################################################
        #
        # Make sure the user specified 100% or less of the disk (if used). -B-
        ...
        if (("$unit_of_measurement" eq "%")
            ...
            #
            # Primary partitions. -B-
            #
            my $p_sum = 0;
            foreach my $m (sort (keys ( %{$xml_config->{disk}->{$dev}->{part}} ))) {

                #
                # Skip over logical partitions. -B-
                # 
                $_ = $xml_config->{disk}->{$dev}->{part}{$m}->{p_type};
                if ( $_ ne "primary" ) { next; }

                #
                # Skip over if size is end_of_disk (*) -- we can't measure that without the disk. -B-
                #
            ...
            # we only need to process this loop if an extended partition exists. -B-
            #
            my $l_sum = 0;
            if ($e_sum > 0) {
                foreach my $m (sort (keys ( %{$xml_config->{disk}->{$dev}->{part}} ))) {
                    ...
                    #
                    # Skip over primary and extended partitions. -B-
                    # 
                    $_ = $xml_config->{disk}->{$dev}->{part}{$m}->{p_type};
            ...
            #
            # Produce error message if necessary. -B-
            #
        ...
        foreach my $m (sort (keys ( %{$xml_config->{disk}->{$dev}->{part}} ))) {
            ...
            # Calculate $startMB and $endMB. -B-
            if ("$p_type{$m}" eq "primary") {
                $startMB{$m} = q($D_OF_LAST_PRIMARY);
            
            } elsif ("$p_type{$m}" eq "extended") {
                $startMB{$m} = q($D_OF_LAST_PRIMARY);
            
            } elsif ("$p_type{$m}" eq "logical") {
                # $startMB{$m} = q($D_OF_LAST_LOGICAL);
                # Fix parted extended partition table kernel reload error: -OL-
                # "Warning:  kernel was unable to re-read the partition table..."
                # Maybe related to bug https://bugzilla.redhat.com/show_bug.cgi?id=441244
                # => TPORARY FIX until parted get fixed.
                $startMB{$m} = q#$(( $D_OF_LAST_LOGICAL + 1 ))#;
            }

            if (("$unit_of_measurement" eq "mb") 
                or ("$unit_of_measurement" eq "megates")) {
                ...
                $endMB{$m} = q#$(echo "scale=3; (# . qq#$startMB{$m}# . q# + ($DISK_SIZ* # . qq#$size{$m} / 100))" | bc)#;
            }

        }
        ### D Populate the simple hashes. -B- ###

        # Figure out what the highest partition number is. -B-
        ...
        ### BIN For empty partitions, change $endMB appropriately. -B- ###
        #
        ...
        until ($m == 0) {
          unless ($endMB{$m}) {
            ...
            $endMB{$m} = '$(( $DISK_SIZ- ' . "$MB_from_end_of_disk" . ' ))';
            $MB_from_end_of_disk++;

            $startMB{$m} = '$(( $DISK_SIZ- ' . "$MB_from_end_of_disk" . ' ))';
            $MB_from_end_of_disk++;
            ...
            $minors_to_remove{$m} = 1;  # This could be any value.  -B-
          }

          $m--;
        }

        # For partitions that go to the end of the disk, tell $endMB to grow to end of disk. -B-
        foreach $m (keys %endMB) {
            if (($size{$m}) and ( $size{$m} eq "*" )) {
                $endMB{$m} = '$(( $DISK_SIZ- ' . "$MB_from_end_of_disk" . ' ))';
            }
        }
        ### D For empty partitions, change $endMB appropriately. -B- ###


        # Start out with a minor of 1.  We iterate through all minors from one 
        # to $highest_part_num, and fool parted  creating bogus partitions
        # where there are gaps in the partition numbers, then later removing them. -B-
        #
        $m = "0";
        until ($m > $highest_part_num) {
            ...
            # after an extended partition, but before logical partitions. -B-
            #
            unless ($endMB{$m}) { next; }
            
            ### Print partitioning commands. -B-
            print $out "\n";
            ...
            if($p_type{$m} eq "extended") {
                ...
                $cmd = qq(parted -s -- $devfs_dev mkpart $p_type{$m} $swap) . q($START_MB $D_MB) . qq( || shellout);
            ...
            # Leave info behind for the next partition. -B-
            if ("$p_type{$m}" eq "primary") {
                print $out q(D_OF_LAST_PRIMARY=$D_MB) . qq(\n);
                print $out q(sleep 1) . qq(\n);
            
            } elsif ("$p_type{$m}" eq "extended") {
                print $out q(D_OF_LAST_PRIMARY=$D_MB) . qq(\n);
                print $out q(D_OF_LAST_LOGICAL=$START_MB) . qq(\n);
            
            } elsif ("$p_type{$m}" eq "logical") {
                print $out q(D_OF_LAST_LOGICAL=$D_MB) . qq(\n);
            }
            ...
            # with n/a values "-" and no spaces in the flags. -B-
            #
            if (
                  ($label_type eq "gpt") 
                  and ($p_name{$m}) 
                  and ($p_name{$m} ne "-")
              ) {  # We're kinda assuming no one names their partitions "-". -B-
            ...
            ### Deal with flags for each partition. -B-
            if(($flags{$m}) and ($flags{$m} ne "-")) {
                ...
                foreach my $flag (@flags) {
                    # Parted 1.6.0 doesn't seem to want to tag gpt partitions with lba.  Hmmm. -B-
        ...
        # Kick the minors out.  (remove temporary partitions) -B-
        ...
        print $out qq(logmsg "$cmd"\n);
        print $out "$cmd\n";
        print $out "### D partition $devfs_dev ###\n";
        print $out "\n";
        print $out "\n";
    }
}
...
sub _read_partition_info_and_prepare_soft_raid_devs {
    ...
    my $xml = XMn($file, keyattr => { raid => "+name" }, forcearray => 1 );
    ...
    foreach my $md ( sort (keys %{$xml->{raid}}) ) {
        ...
        foreach (@md_devices) {
            # m/^(.*)(p?\d+)$/;
            # 
            # New regex from patch provided  Thomas Zeiser <thomas.zeiser@rrze.uni-erlangen.de>
            #   Hi,
            #   
            ...
            my $disk = $1;
            my $part_no = $2;
            $devices .= '${' . $DISK__disk{$disk} . '}' . $part_no . ' ';
        }
    ...
    #
    #XXX
    #   - for DIC we can literally list every device involved.  Ie:
    #       DIC/dev/sda1 /dev/sdb1 /dev/sdc1 etc...
...
sub _read_partition_info_and_prepare_pvcreate_commands {
    ...
    foreach my $dev (sort (keys ( %{$xml_config->{disk}} ))) {
        ...
        until ($m >= $highest_part_num) {
            ...
            # tended partitions can't be used  LVM. -AR-
                        ...
                        foreach my $lvm (@{$xml_config->{lvm}}) {
                            ...
                            foreach my $lvm_group_name (@{$lvm->{lvm_group}}) {
                                if ($lvm_group_name->{name} eq $vg_name) {
                                    $cmd = "Initializing partition $part for use  LVM.";
                                    print $out qq(logmsg "$cmd"\n);
...
sub write_lvm_groups_commands {
    ...
    my $xml_config = XMn($file, keyattr => { lvm_group => "+name" }, forcearray => 1 );
    ...
    foreach my $lvm (@{$xml_config->{lvm}}) {
        ...
        foreach my $group_name (sort (keys ( %{$lvm->{lvm_group}} ))) {
            ...
            foreach my $disk (@{$xml_config->{disk}}) {
                ...
                # aluate the partition list for the current LVM group -AR-
                my $m = "0";
                foreach my $part (@{$disk->{part}}) {
                    $m++;
           ...
           # Find RAID disks assigned to the volume group.
            ...
            if ($part_list ne "") {
                # aluate the volume group options -AR-
...
sub upgrade_partition_schemes_to_generic_style {
    ...
    #
    my @disk_types = qw( . rd ida cciss );  #  . is for ide and scsi disks. -B-
            ...
            opendir(DIR, $dir) || die "Can't read the $dir directory.";
                while(my $device = readdir(DIR)) {
                    ...
                    if (-f $file) {
                        ...
                        Rsyphon::Common->save_partition_information($file, "old_sfdisk_file", $autoinstall_script_conf_file);
                    }
                }
            close(DIR);
        }
...
# Description:
# Read configuration information from /etc/rsyphon/autoinstallscript.conf
# and write filesystem creation commands to the autoinstall script. -B-
#
# Usage:
...
sub _write_out_mkfs_commands {
    ...
    my $xml_config = XMn($file, keyattr => { fsinfo => "+line" }, forcearray => 1 );
    ...
    # Figure out if software RAID is in use. -B-
    #
    my $software_raid;
    foreach my $line (sort numerically (keys ( %{$xml_config->{fsinfo}} ))) {

        # If this line is a comment, skip over. -B-
        if ( $xml_config->{fsinfo}->{$line}->{comment} ) { next; }

        # If real_dev isn't set, move on. -B-
    ...
    #  $part_type here is used to find and create all the swap partitions
    ...
    foreach my $part_type (0, 1) {
        foreach my $line (sort numerically (keys (%{$xml_config->{fsinfo}}))) {
            ...
            # If format="no" is set, then skip over this one. -B-
            my $format = $xml_config->{fsinfo}->{$line}->{format};
            if (($format) and ( "$format" eq "no")) { next; }

            # mount_dev should contain fs LAB or UUID information. -B-
...
# Description:
# Validate information in $auto_install_script_conf. -B-
#
...
sub validate_auto_install_script_conf {
    ...
    ############################################################################
    #
    # Don't allow duplicate line numbers in the fsinfo section. -B-
    #
    ############################################################################
    my %nodups;
    my $xml_config = XMn($file, forcearray => 1 );
    foreach my $hash ( @{$xml_config->{fsinfo}} ) {

        $_ = ${$hash}{'line'};

        if ($nodups{$_}) {
            print qq(Doh!  re's more than one line numbered "$_" in "$file"!\n);
...
# immediately after pulling down the image. -B-
#
# Usage:
...
sub _write_out_new_fstab_file {
    ...
    print $out qq(cat <<'F' > /a/etc/fstab\n);
    ...
    foreach my $line (sort numerically (keys ( %{$xml_config->{fsinfo}} ))) {
        ...
        # Update the root device. This will be used  systemconfigurator
        # (see below).
        if ($mp eq '/') {
            $rootdev = $mount_dev;
        } elsif ($mp eq '/boot') {
        ...
        if ($real_dev) {
            ...
            print STDR "WARNING: real_dev is not defined for $mount_dev!\n";
        }
...
sub _write_out_umount_commands {
    ...
    # mp entry.  Associate filesystems  mount points in a hash here, then we
    # can reverse sort  mount point below to unmount them all. -B-
    #
    my %fs__mp;   
    foreach my $line (reverse sort (keys ( %{$xml_config->{fsinfo}} ))) {
        ...
        if ( $xml_config->{fsinfo}->{$line}->{fs} ) { 
            ...
            # therefore not mounted, to the list of filesystems to umount. -B-
            #
            my $format = $xml_config->{fsinfo}->{$line}->{format};
            if (($format) and ( "$format" eq "no")) { next; }
            ...
            # 
            # Don't include in hash below unless it's a supported filesystem. -B-
            #
            unless( 
                      ($fs eq "ext2") 
                   or ($fs eq "ext3") 
    ...
    #
    $xml_config = XMn($file, keyattr => { boel => "+devstyle"} );
    ...
    # -B-
    #
    foreach my $mp (reverse sort (keys ( %fs__mp ))) {
...
#        $bootdev_disk =~ s/[0-9]+$//;
#    } elsif ($bootdev_disk =~ /^UUID|^LAB/) {
#        # XXX: Boot device in UUID or LAB form: do nothing,
#        # systemconfigurator will do everything is needed.
#    } else {
#        # Hardware RAID device.
#        $bootdev_disk =~ s/p[0-9]+$//;
#    }
#    print $out "    sed -i 's:[[:space:]]*BOOTD[[:space:]]*=.*:BOOTD = $bootdev_disk:g' $sc_conf_file\n";
#    print $out "    sed -i 's:[[:space:]]*ROOTD[[:space:]]*=.*:ROOTD = $rootdev:g' $sc_conf_file\n";
#    print $out "    sed -i 's:[[:space:]]*root=[^ \\t]*: root=$rootdev :g' $sc_conf_file\n";
#    print $out "    sed -i \"s:DAULTBOOT = rsyphon:DAULTBOOT = \$IMAGAMg\" $sc_conf_file\n";
#    print $out "    sed -i \"s:LAB = rsyphon:LAB = \$IMAGAMg\" $sc_conf_file\n";
#    print $out "fi\n";
#}
...
#    print $out "[ -z \$DIC] && DICeth0\n";
#
#    my $sc_excludes_to = "/etc/rsyphon/systemconfig.local.exclude";
...
#        $sc_ps3_options = '';
#    } else {
#        ## FIXM- is --excludesto only for the static method?
...
#    print $out "fi\n";
#    print $out "$sc_cmd \${sc_options} --stdin << L || shellout\n";
#
#    unless ($ip_assignment_method eq "replicant") {
#	print $out "[NWORK]\n";
#	print $out "HOSTNAM= \$HOSTNAMn";
#	print $out "DOMAINNAM= \$DOMAINNAMn";
#    }
#    if ($ip_assignment_method eq "static") {
#	print $out "GATAY = \$GATAY\n";
#    }
#    print $out "\n";
#
#    print $out "[INTFAC]\n";
...
#	print $out "TYP= static\n";
#	print $out "IPADDR = \$IPADDR\n";
#	print $out "NMASK = \$NMASK\n";
#    }
#
#    print $out "L\n";
#}
...
sub append_variables_txt_with_ip_assignment_method {
    ...
    #   dhcp
    #
    print $out "echo IP_ASSIGNMT_MHOD=$ip_assignment_method >> /tmp/variables.txt\n\n";
}
...
sub create_autoinstall_script{
    ...
    # don't really need it for anything anyway. -B-
    #
    ...
    my $template = "/etc/rsyphon/autoinstallscript.template";
    open (my $TPLAT "<$template") || die "Can't open $template for reading\n";
    open (my $MAST_SCRIPT, ">$file") || die "Can't open $file for writing\n";
    ...
    while (<$TPLAT) {
        SWITCH: {
	        if (/^\s*${delim}VSION_INFO${delim}\s*$/) {
	            print $MAST_SCRIPT "# This master autoinstall script was created with Rsyphon v${VSION}\n";
	            last SWITCH;
	        }

	        if (/^\s*${delim}S_IMAGAM{delim}\s*$/) {
                print $MAST_SCRIPT  q([ -z $IMAGAM] && ) . qq(IMAGAM$image\n);
	            last SWITCH;
	        }

	        if (/^\s*${delim}S_OVRID${delim}\s*$/) {
                if ($overrides) {
                    $overrides =~ s/,/ /g;
                    my @list = ();
                    foreach (split(/ /, $overrides)) {
                        if (-d $::main::config->default_override_dir() . '/' . $_) {
                            ...
                            print STDR "WARNING: override $_ doesn't exist! (skipping)\n";
                        }
                    }
                    $overrides = join(' ', @list);
                } else {
                ...
                print $MAST_SCRIPT  q([ -z $OVRID ] && ) .
                                      qq(OVRID="$script_name \$GROUP_OVRID \$HOSTNAM$overrides"\n);
	            last SWITCH;
	        }

	        if (/^\s*${delim}S_DISKORD${delim}\s*$/) {
                    # Set or unset disk autodetection.
                    if ($autodetect_disks) {
                        print $MAST_SCRIPT qq(DISKORD=sd,cciss,ida,rd,hd,xvd\n);
                    } else {
                        print $MAST_SCRIPT qq(DISKORD=\n);
                    }
	            last SWITCH;
	        }

	        if (/^\s*${delim}PARTITION_DISKS${delim}\s*$/) { 
	            _read_partition_info_and_prepare_parted_commands( $MAST_SCRIPT,
	          						$image_dir, 
	          						$auto_install_script_conf);
	            last SWITCH;
	        }

                if (/^\s*${delim}CRTSOFT_RAID_DISKS${delim}\s*$/) { 
                _read_partition_info_and_prepare_soft_raid_devs( $MAST_SCRIPT,
                    $image_dir, 
                    $auto_install_script_conf);
                    last SWITCH;
                }

                if (/^\s*${delim}INITIALIZLVM_PARTITIONS${delim}\s*$/) {
 	            _read_partition_info_and_prepare_pvcreate_commands( $MAST_SCRIPT,
 	          						$image_dir,
 	          						$auto_install_script_conf);
                     last SWITCH;
                }

                if (/^\s*${delim}CRTLVM_GROUPS${delim}\s*$/) {
                    write_lvm_groups_commands( $MAST_SCRIPT,
                                            $image_dir,
                                            $auto_install_script_conf);
                     last SWITCH;
                }

                if (/^\s*${delim}CRTLVM_VOLUM${delim}\s*$/) {
                     write_lvm_volumes_commands( $MAST_SCRIPT,
                                            $image_dir,
                                            $auto_install_script_conf);
                     last SWITCH;
                }
	        
                if (/^\s*${delim}CRTFILYSTS${delim}\s*$/) {
	            _write_out_mkfs_commands( $MAST_SCRIPT, 
	          			$image_dir, 
	          			$auto_install_script_conf);
	            last SWITCH;
	        }

	        if (/^\s*${delim}GATFSTAB${delim}\s*$/) {
                append_variables_txt_with_ip_assignment_method( $MAST_SCRIPT, $ip_assignment_method );
	            _write_out_new_fstab_file( $MAST_SCRIPT, 
	          			 $image_dir, 
	          			 $auto_install_script_conf );
	            last SWITCH;
	        }
	        ...
	        if (/^\s*${delim}NO_LISTING${delim}\s*$/) {
	            unless ($listing) { print $MAST_SCRIPT "NO_LISTING=yes\n"; }
	            last SWITCH;
	        }
            
            #if (/^\s*${delim}BO_DSTYL{delim}\s*$/) {
            #_write_boel_devstyle_entry($MAST_SCRIPT, $auto_install_script_conf);
            #    #    last SWITCH;
            #}

            #if (/^\s*${delim}SYSTCONFIGURATOR_PR{delim}\s*$/) {
            #    write_sc_command_pre($MAST_SCRIPT, $ip_assignment_method);
            #    last SWITCH;
            #}

            #if (/^\s*${delim}SYSTCONFIGURATOR_POST${delim}\s*$/) {
            #    write_sc_command_post($MAST_SCRIPT, $ip_assignment_method);
            #    last SWITCH;
            #}

	        if (/^\s*${delim}UMOUNT_FILYSTS${delim}\s*$/) {
	            _write_out_umount_commands( $MAST_SCRIPT,
	          			  $image_dir, 
	          			  $auto_install_script_conf );
	            last SWITCH;
	        }
	        ...
	        if (/^\s*${delim}MONITOR_POSTINSTALL${delim}\s*/) {
                    ...
                    print $MAST_SCRIPT "    send_monitor_msg \"status=$post_state->{$post_install}:speed=0\"\n";
                    last SWITCH;
                }
	        ...
	        if (/^\s*${delim}POSTINSTALL${delim}\s*/) {
                ...
                if ($post_install eq "beep") {
                    ...
                    print $MAST_SCRIPT "beep_incessantly";
                } elsif ($post_install eq "reboot") {
                    # reboot stuff
                    print $MAST_SCRIPT "# reboot the autoinstall client\n";
                    print $MAST_SCRIPT "shutdown -r now\n";
                } elsif ($post_install eq "shutdown") {
                    # shutdown stuff
                    print $MAST_SCRIPT "# shutdown the autoinstall client\n";
                    print $MAST_SCRIPT "shutdown\n";
                    print $MAST_SCRIPT "\n";
                } elsif ($post_install eq "kexec") {
                    # kexec imaged kernel
                    print $MAST_SCRIPT "# kexec the autoinstall client\n";
                    print $MAST_SCRIPT "# this is executed twice to support relocatable kernels from RH5\n";
                    print $MAST_SCRIPT "kexec --force --append=\"\$kexec_append\" --initrd=/tmp/\$kexec_initrd --reset-vga /tmp/\$kexec_kernel\n";
                    print $MAST_SCRIPT "kexec --force --append=\"\$kexec_append\" --initrd=/tmp/\$kexec_initrd --reset-vga --args-linux /tmp/\$kexec_kernel\n";
                }
                last SWITCH;
	        }

			if (/^\s*${delim}SHOW_DISK_ITS${delim}\s*$/) {
				show_disk_edits( $MAST_SCRIPT );
				last SWITCH;
			}
			if (/^\s*${delim}IT_DISK_NAM${delim}\s*$/) {
				edit_disk_names( $MAST_SCRIPT );
				last SWITCH;
			}

			if (/^\s*${delim}SUP_K${delim}\s*$/) {
                if ($post_install eq "kexec") {
                    setup_kexec( $MAST_SCRIPT );
                } else {
                    print $MAST_SCRIPT "# Not needed for this post-install action\n";
                }
                last SWITCH;
            }

	        ### D end of autoinstall options ###
	        print $MAST_SCRIPT $_;
        }
    }
    close($TPLAT;
    ...
    ### BIN overrides stuff ###
    # Create default overrides directory. -B-
    #
    ...
    if (! -d "$dir")  {
      ...
      #
      #  best solution here is to use the same permission mask of the image.
...
sub _remove_mount_option {
    ...
    # so we return "defaults" rather than a null value. -B-
    unless($new_options) {
...
sub copy_boot_files_to_boot_media {
    ...
    my $message_txt = "/etc/rsyphon/pxelinux.cfg/message.txt";
    my $syslinux_cfg = "/etc/rsyphon/pxelinux.cfg/syslinux.cfg";
    ...
    unless ($append_string) {
        ...
        # Append to APPD line in config file.
        my $infile = "$syslinux_cfg";
        my $outfile = "$mnt_dir/syslinux.cfg";
        open(INFIL"<$infile") or croak("Couldn't open $infile for reading.");
            open(OUTFIL">$outfile") or croak("Couldn't open $outfile for writing.");
                while (<INFIL) {
                    if (/^\s*APPD\s+/) { 
                        chomp;
		      # mit of 255 specified in Documentation/i386/boot.txt
                        $_ = $_ . " $append_string\n";
    ...
    if ($arch eq "ia64") {
        ...
        my $efi_dir = Rsyphon::Common->where_is_my_efi_dir();
        ...
        if (-f $elilo_efi) {
            ...
            print "the location of your elilo.efi file, at: http://rsyphon.org/support/\n\n";
            die;
        }
...
#XXX why are we creating a new elilo.conf file?  Shouldn't we work with the one the system provides?  -B-
# Description:
...
sub _write_elilo_conf {
        ...
        open(ILO_CONF,">$file") or croak("Couldn't open $file for writing.");
        
                print ILO_CONF "timeout=20\n";
                
                if ($append_string) { 
                        print ILO_CONF qq(append="$append_string"\n);
                }
                
                print ILO_CONF "image=kernel\n";
                print ILO_CONF "  label=linux\n";
                print ILO_CONF "  read-only\n";
                print ILO_CONF "  initrd=initrd.img\n";
                print ILO_CONF "  root=/dev/ram\n";
...
#    my ($script, $file) = @_;
#
#    my $xml_config = XMn($file, keyattr => { boel => "+devstyle"} );
#
#    if( defined($xml_config->{boel}->{devstyle}) 
