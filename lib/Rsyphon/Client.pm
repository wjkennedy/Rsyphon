...
#
#    2001-2014    
#    2001- Sean Dague <sean@dague.net>
#
...
use Rsyphon::Config;
use base qw(porter);
use vars qw(@PORT @PORT_OK %PORT_TAGS $VSION);

$VSION = "SYSTIMAG_VSION_STRING";

@PORT_OK = qw(client_exists client_info addclient removeclient);
%PORT_TAGS = ('all' => [@PORT_OK]);

our $config = $Rsyphon::Config::config;
...
sub _client_ip {
    ...
    while(my ($fullname, $aliases, $addrtype, $length, @addrs) = gethostent()) {
        ...
        if($name eq $fullname or _in($name, @aliases)) {
            ...
            #  unpack is taken from pg 721 Camel 3
