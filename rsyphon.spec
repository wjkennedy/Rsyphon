...
- Reverted rsnetbootmond wrong fix and fixed the man instead.

* Tue Jul 17 2014 Olivier Lahaye <olivier.lahaye@cea.fr> 4.3.0-0.18
- Fix rsnetbootmond that refused to do its job.
...
- New beta version. (fix rsmonitortk thread warning)
...
- Updated rsyphon_server_pm.patch (use $(()) for arithmetics)
...
- Removed rsyphon-client dependency from rsyphon-initrd-template
...
- Include dir /etc/rsyphon/kboot.cfg

* Tue May 22 2007 Bernard Li <bernard@vanhpc.org>
- Fixed typo: rsyphon-server-rsyncd -> rsyphon-server-monitord for upgrade service restart
...
- added pattern exclusions for rsgetimage in /etc/rsyphon/getimage.exclude

* Thu Mar 08 2007 Andrea Righi <a.righi@cineca.it>
- Added rspushoverrides command.
...
- Added missing directories to filelist for rsyphon-server
...
- Updated URLs to http://wiki.rsyphon.org 

* Wed Aug 02 2006 Bernard Li <bli@bcgsc.ca>
- Officially taking over as packager of Rsyphon RPMs
...
- Added %doc README.Rsyphon_DHCP_options, README.ssh_support and
  TODO to rsyphon-server package

* Mon Jun 11 2006 Bernard Li <bli@bcgsc.ca>
- New package: rsyphon-imagemanip

* Fri Jun 09 2006 Bernard Li <bli@bcgsc.ca>
- Added file /etc/rsyphon/UYOK.modules_to_include

* Fri Apr 21 2006 Bernard Li <bli@bcgsc.ca>
- New package: rsyphon-bittorrent
...
- Added bits to add/remove init scripts for rsyphon-server-{netbootmond
  monitord,bittorrent}
- Added /usr/share/rsyphon/icons/* to %files
...
- Added general description text for rsyphon package as this is used by SRPM

* Thu Nov 17 2005 Bernard Li <bli@bcgsc.ca>
- Added ./configure RS_BUILD_DOCS=1 to ensure building of docs
...
- Added directory /var/lock/rsyphon
...
- Here is another patch for RPM building. With this patch, you should be
  ...
  -ba rsyphon.spec") and get a full set of RPMs. I assume you want
...
- html documentation returned to rsyphon-server package
...
- add version dependency for rsyphon-flamethrower package
...
- add missing conf file & state dir to rsyphon-server-flamethrower
...
- make rsyphon-flamethrower depend on flamethrower
...
if [ -z $BT_TRACKER_BIN ]; then
	...
	echo "--> For details, please see http://wiki.rsyphon.org/index.php/Quick_Start_HOWTO"
else
	echo done
fi
...
%config(noreplace) %{_sysconfdir}/rsyphon/rsyphon.conf
...
%{_sbindir}/rsaddclients
%{_sbindir}/rscpimage
%{_sbindir}/rsgetimage
%{_sbindir}/rsmk*
%{_sbindir}/rsmvimage
%{_sbindir}/rsnetbootmond
%{_sbindir}/rspushupdate
%{_sbindir}/rspushinstall
%{_sbindir}/rsrmimage
%{_sbindir}/rsmonitor
%{_sbindir}/rsmonitortk
%{_bindir}/rsclusterconfig
%{_bindir}/rsmk*
%{_bindir}/rspsh
%{_bindir}/rspcp
%{_bindir}/rspushoverrides
...
%{_sbindir}/rsupdateclient
%{_sbindir}/rsprepareclient
%{_mandir}/man8/rsupdateclient*
%{_mandir}/man8/rsprepareclient*
